...
public class AuthManager {
    ...
    public boolean validToken(HttpServletRequest request, HttpServletResponse response) throws IOException {
        try {
            ...
            if (token != null){
                Optional<Usuario> appUsuario = usuarios.findByUsername(provider.getUserFromToken(token));
                if (appUsuario.isPresent()) return true;
            }  else response.sendError(403, "Token vacío");
